...
let items = [
  ...
  'I want to join the affliate program',
];
...
const LetStart: React.FC = () => {
  ...
  const { updateApplicationData } = useInternshipInfo();
  const { handleNext } = useInternshipApplyHandlers();
      ...
      <div className="w-[305px] mt-[150px] md:mt-[initial]">
        ...
        {items.map((e, i) => (
          <div
            ...
            onClick={() => setRole(i)}
            className={`
                            py-[15px] px-[24px] hover:border-red rounded-[40px] mb-[18px] w-full border-[2px] border-[#949494] text-center cursor-pointer text-[16px] leading-[22px] font-[400]
                            ${
                              role === i
