            ...
            <div className='w-full min-h-[75%] flex items-center justify-center lg:justify-start px-[60px] md:px-0 lg:pl-[60px]' >
                <div>
                    ...
                    <div className='mb-[25px]' >
                        <h3 className='text-[16px] leading-[22px] font-[500] text-[#1D1D1D] mb-[10px]' >Your Full Name</h3>

                        <input
                            name='fullname'
                            className={`w-[360px] md:w-[540px] h-[56px] p-[20px] bg-transparent outline-none text-[16px] rounded-[8px] border ${nameErr ? 'border-primary' : 'border-[#cfcfcf]'}`}
                            placeholder='Gregg'
                            onChange={handleChange}
                            value={fullname}
                        />
                        {nameErr && <small className='text-primary mt-[7px] block text-end' >* Required Field</small>}
                    </div>

                    <div className='mb-[25px]' >
                        <h3 className='text-[16px] leading-[22px] font-[500] text-[#1D1D1D] mb-[10px]' >Your Email Address</h3>

                        <input
                            name='email'
                            className={`w-[360px] md:w-[540px] h-[56px] p-[20px] bg-transparent outline-none text-[16px] rounded-[8px] border ${emailError || emailErr ? 'border-primary' : 'border-[#cfcfcf]'}`}
                        ...
                        <small className='block text-primary text-end mt-[7px]' >
                            {emailError && "Email is invalid" || emailErr && "* Required Field"}
                        </small>
                    </div>

                    <div>
                        <h3 className='text-[16px] leading-[22px] font-[500] text-[#1D1D1D] mb-[10px]' >Your Phone Number</h3>

                        <input
                            name='phone'
                            className={`w-[360px] md:w-[540px] h-[56px] p-[20px] bg-transparent outline-none text-[16px] rounded-[8px] border ${phoneErr ? 'border-primary' : 'border-[#cfcfcf]'}`}
                            placeholder='(+123) 456 789 88'
                            onChange={handleChange}
                            value={phone}
                        />
                        {phoneErr && <small className='text-primary mt-[7px] block text-end' >* Required Field</small>}
                    </div>
