        ...
        <>
            ...
            <div className='w-full min-h-[75%] flex items-center justify-center lg:justify-start px-[60px] md:px-0 lg:pl-[60px]' >
                <div className='my-[90px] lg:my-[0px]' >
                    <div>
                        <h3 className='text-[16px] leading-[16px] font-[400] text-[#1D1D1D] mb-[15px] pl-[65px] md:pl-[95px]' >Enter your monthly pay goal.</h3>

                        <div className='flex gap-[15px] md:gap-[30px] items-center' >
                            <button className={`w-[50px] h-[50px] md:w-[65px] md:h-[65px] transition rounded-full border flex items-center justify-center ${amount < 1 ? "border-[#cfcfcf] text-[#cfcfcf]" : "border-[#1D1D1D] text-[#1D1D1D] hover:border-primary hover:text-primary"} cursor-pointer`} onClick={() => handleDecrease()} >
                                <AiOutlineMinus fontSize='28px' />
                            </button>

                            <div className='relative' >
                                <div className={`w-[230px] md:w-[350px] p-[17px] md:p-[20px] h-[56px] flex items-center gap-[5px] rounded-[8px] border ${showError ? 'border-primary' : 'border-[#cfcfcf]'}`} >
                                    {amount > 0 && <span className='noselect' >$</span>}

                                    <input
                                        value={amount}
                                        className={`bg-transparent noselect outline-none text-[16px] m-0 p-0 w-full`}
                                        placeholder='$0.00'
                                        onChange={handleChange}
                                    />
                                </div>
                                {showError && <small className='text-primary mt-[7px] block absolute right-0' >* Required Field</small>}
                            </div>

                            <button className='w-[50px] h-[50px] md:w-[65px] md:h-[65px] transition hover:border-primary hover:text-primary rounded-full border flex items-center justify-center border-[#1D1D1D] cursor-pointer' onClick={() => handleIncrease()} >
