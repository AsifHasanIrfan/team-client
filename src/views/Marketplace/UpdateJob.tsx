...
const UpdateJob = () => {
  ...
  const query = `marketplaceId=${id}&user=${auth?.user?._id}`;
  // hooks
  ...
  const { applyProjects, applyProjectsLoading, applyProjectsFetch } =
    useApplyProjects(auth?.token, query);

  useEffect(() => {
    // if (applyProjects?.datas?.length) {
      setApplyData({
        isDgTeamWillHandle: applyProjects?.datas[0]?.isDgTeamWillHandle,
        reason: applyProjects?.datas[0]?.whyPerfect,
        attachment: applyProjects?.datas[0]?.attachments,
        bidAmount: 0,
      });
      setSelectedPaymentMethod(
        paymentMethodOptions[
          paymentMethodOptions.findIndex(
            (item) => item.value === applyProjects?.datas[0]?.paymentMethod
          )
        ]
      );
      setPaymentMethodUrl({
        label: applyProjects?.datas[0]?.paymentMethod,
        value: applyProjects?.datas[0]?.paymentURL,
      });
    // }
  }, [applyProjects?.datas]);
  ...
  if (applyProjects?.datas?.length === 0) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    let applyDatas = {
      ...
      attachments: [...attachments, ...applyData?.attachment],
      paymentNumber: '',
      paymentURL: '',
    ...
    axios
      // .patch(
      //   `http://localhost:4000/api/proposal/apply/update/${applyProjects?.datas[0]?._id}`,
      .patch(
        `${process.env.serverUrl}proposal/apply/update/${applyProjects?.datas[0]?._id}`,
        applyDatas,
        {
          headers: {
      ...
      .then((response) => {
        ...
        if (response.data.success === true) {
          ...
          marketplaceFetch();
          router.push('/dashboard/marketplace');
          return toast.success('Proposal updated successfully!');
        }
      });
  };
  ...
  // loading
  if (!auth.token || userLoading || marketplaceLoading || applyProjectsLoading)
  ...
  return (
    <div className="text-[#454545]">
      <form onSubmit={handleSubmit}>
        ...
        <div className="custom-checkbox w-max" role="button">
          ...
          >
            ...
            <input
              type="checkbox"
              id="terms"
              checked={applyData.isDgTeamWillHandle}
        ...
        <AttachmentUpladSection
          getAttachment={getAttachment}
          appliedAttachment={applyProjects?.datas[0]?.attachments}
        />
        ...
        <div className="flex items-center md:flex-row flex-col justify-end gap-[15px] mt-5">
          <ApplyButton
            type="button"
